...
if check_password():
    ...
    def run(rms, adr, nts, mix, cp, fl, ev_rev=0, total_tr_cost=0):
        ...
        # New: Inventory Displacement calculation
    ...
    def seg(nm, cl, bg, kp, ad_d, fl_d, cp, is_group=False):
        ...
        with c2:
            ...
            q = {"RO": ca.number_input("RO",0,key=kp+"ro"), "BB": ca.number_input("BB",0,key=kp+"b"), "HB": cb.number_input("HB",0,key=kp+"h"),
                 "FB": cb.number_input("FB",0,key=kp+"f"), "SAI": cc.number_input("SAI",0,key=kp+"sa"), "AI": cc.number_input("AI",0,key=kp+"ai")}
            if is_group:
                cx, cy = st.columns(2)
                ev_r, tr_c = cx.number_input("Event/Pax",0.0,key=kp+"ev"), cy.number_input("Trans
