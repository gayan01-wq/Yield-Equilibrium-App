...
with st.sidebar:
    ...
    currencies = [
        "OMR", "AED", "SAR", "KWD", "BHD", "QAR", "JOD", "EGP", # Middle East
        "LKR", "INR", "PKR", "BDT", "THB", "SGD", "MYR", "CNY", "JPY", "KRW", "IDR", "HKD", "VND", "PHP", # Asia
...
def calc_w(rms, adr, n, meals, comm, fl, ev=0.0, tr=0.0):
    ...
    if crisis_active:
        if unit_w > 2.0:
            ...
            msg = f"<b>📉 CRISIS STRATEGY:</b> ACCEPT. Contribution to fixed labor: {cu} {unit_w:,.2f}."
        elif unit_w > 0:
            l, b = "RAZOR THIN", "#f1c40f"
            msg = "<b>⚠️ CRITICAL VOLUME:</b> Breakeven reached. Use for base inventory only."
        else:
            l, b = "REJECT", "#e74c3c"
            msg = f"<b>🛑 REJECT:</b> Even in Crisis, this deal loses {cu} {abs(total_w):,.0f}."
    else:
        util = (tot_rms / h_total) * 100
        hurdle = fl * 1.25 if util >= 20.0 else fl
        if unit_w < (hurdle * 0.95):
            l, b = "DILUTIVE", "#e74c3c"
            msg = f"<b>🚩 REJECT:</b> Fails Yield Hurdle ({cu} {abs(unit_w - hurdle):,.2f})."
        elif unit_w < hurdle:
            l, b = "MARGINAL", "#f1c40f"
            msg = "<b>⚠️ FILLER ONLY:</b> Acceptable for inventory absorption, not for core strategy."
        else:
            l, b = "OPTIMIZED", "#27ae60"
            msg = f"<b>💎 ACCEPT:</b> Wealth Generator surplus against market floor."
...
with col_p1: st.markdown("<div class='pillar-box'><h4>1. Wealth Stripping</h4><p>Isolating bankable cash by removing taxes and variable costs.</p></div>", unsafe_allow_html=True)
with col_p2: st.markdown("<div class='pillar-box'><h4>2. Capacity Sensitivity</h4><p>Dynamic hurdles triggered at 20% utilization baseline.</p></div>", unsafe_allow_html=True)
with col_p3: st.markdown("<div class='pillar-box'><h4>3. Efficiency Indexing</h4><p>Measuring Net Wealth variance against operational breakeven.</p></div>", unsafe_allow_html=True)
