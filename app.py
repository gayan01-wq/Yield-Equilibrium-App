...
with st.sidebar:
    ...
    inventory = st.number_input("Total Capacity", 1, 1000, 237, key="inv_total_"+rk)
    ...
    meal_costs = {
        ...
        "SAI": st.number_input("SAI Cost (Soft AI)", 8.5, key="sai_cost_"+rk),
...
def run_yield(rms, nts, adr, meals, hurdle, demand_type, comm_rate=0.0, laundry=0, mice=0, trans=0, snack_qty=0):
    ...
    # Apply Demand Hurdle Adjustment
    ...
    if unit_w < effective_hurdle: stt, clr, rsn = "REJECT: DILUTIVE", "#e74c3c", f"Yield < {cur_sym}{effective_hurdle} hurdle for {demand_type}."
...
def draw_seg(label, key, suggest_adr, floor_def, color, is_ota=False, group=False):
    ...
    with c_in:
        ...
        demand_sel = m_row[0].selectbox("Demand Context", ["Compression (Peak)", "High Flow", "Standard", "Distressed"], key=f"dem_{key}_{rk}")
        ...
        if group:
            g_row = st.columns(4)
            m_c = g_row[0].number_input(f"MICE Revenue", 0.0, key=f"mi_{key}_{rk}"); t_c = g_row[1].number_input(f"Trans Revenue", 0.0, key=f"tr_{key}_{rk}"); l_c = g_row[2].number_input(f"Laundry Cost", 0.0, key=f"la_{key}_{rk}"); p_snk = g_row[3].number_input("Snack Qty", 0, key=f"sn_{key}_{rk}")
        else:
...
with c_a:
    st.markdown(f"<div class='theory-card'><b>🏗️ Pillar 01: Internal Wealth Stripping</b><br>Stripping Tax (**{tx_div}**), Commissions (**{ota_comm}%**), and Meal Costs (BB, LN, DN, SAI, AI).</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='theory-card'><b>⚖️ Pillar 02: Demand Selection Integration</b><br>The <b>Demand Context</b> selector adjusts the base hurdle. <i>Compression</i> adds +15.0 to protect peak inventory, while <i>Distressed</i> drops it by -5.0 to capture volume.</div>", unsafe_allow_html=True)
with c_b:
    ...
    st.markdown(f"<div class='theory-card'><b>📉 Inventory Logistics</b><br>Captures 100% of deal wealth including MICE, Transportation, and Laundry across SGL/DBL/TPL room units. 50% Displacement check applied.</div>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)
