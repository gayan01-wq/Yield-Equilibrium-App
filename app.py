...
# --- 1. STYLING (Ordered: Layout -> Title -> Subtitle) ---
st.set_page_config(layout="wide", page_title="Smart Profit & Deal Evaluator")
st.markdown("""<style>
.block-container{padding-top:1rem!important;}
.main-title{font-size:1.8rem!important;font-weight:900;color:#1e3799;text-align:center;margin-top:-10px;}
.main-subtitle{font-size:1.1rem!important;font-weight:600;color:#4b6584;text-align:center;margin-top:-15px;margin-bottom:20px;letter-spacing:0.5px;}
...
active_intel = intel_db.get(city_search.lower(), {"ev": "Active Rotation", "fl": "Baseline", "news": ["Standard market."], "basis": "Equilibrium"})
...
# --- 6. DASHBOARD ---
st.markdown("<h1 class='main-title'>SMART PROFIT & DEAL EVALUATOR</h1>", unsafe_allow_html=True)
st.markdown("<div class='main-subtitle'>Advanced Yield & Asset Performance Intelligence Hub</div>", unsafe_allow_html=True)

t1, t2 = st.tabs(["🌐 Aviation & Events", "🗞️ Market News Feed"])
with t1: st.markdown(f"<div class='google-window'><b>🌐 Aviation Intelligence: {city_search}</b><br>• <b>Events:</b> {active_intel['ev']} | <b>Velocity:</b> {v_mult}x Applied</div>", unsafe_allow_html=True)
with t2:
...
with c_a:
    ...
    st.markdown(f"<div class='theory-card'><b>⚖️ Pillar 02: Demand & Hurdle Logic</b><br>The <b>Demand</b> context adjustments: <i>Compression</i> adds +15.0 to protect peak inventory, while <i>Distressed</i> drops it by -5.0.</div>", unsafe_allow_html=True)
with c_b:
    ...
    st.markdown(f"<div class='theory-card'><b>📉 Inventory Logistics</b><br>Captures 100% of deal wealth including MICE, Transportation, and Laundry across SGL/DBL/TPL room units.</div>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)
