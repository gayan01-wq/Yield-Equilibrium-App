...
with st.sidebar:
    ...
    st.divider()
    st.markdown("<p style='font-size: 1.1rem; font-weight: 800; color: #1e3799; margin:0;'>Gayan Nugawela</p><p style='font-size:0.75rem;'>Strategic Revenue Architect</p>", unsafe_allow_html=True)
    st.divider()
...
def calc_w(rms, adr, n, meals, comm, fl, mice=0.0, trans=0.0):
    ...
    unit_w = (u_net - m_cost - ((u_net - m_cost) * comm) - p01) + ((mice * pax_r) / (n * tx))
    total_wealth = (unit_w * tot_rms * n) + (trans / tx)
    disp_u_w = total_wealth / (tot_rms * n)
    if crisis_active:
        l, b, msg = ("SURVIVAL ACCEPT", "#27ae60", "Crisis: Costs Covered") if disp_u_w > 0 else ("REJECT", "#e74c3c", "Below Cost")
    else:
        hurdle = fl * 1.25 if (tot_rms/h_total) >= 0.2 else fl
        if disp_u_w < (hurdle * 0.95): l, b, msg = "DILUTIVE", "#e74c3c", f"REJECT: Fails Hurdle by {abs(disp_u_w - hurdle):,.2f}"
        elif disp_u_w < hurdle: l, b, msg = "MARGINAL", "#f1c40f", "FILLER ONLY"
        else: l, b, msg = "OPTIMIZED", "#27ae60", "ACCEPT"
    return {"u": disp_u_w, "l": l, "b": b, "tot": total_wealth, "rn": tot_rms*n, "msg": msg, "crit": disp_u_w < (0 if crisis_active else hurdle)}

# --- 5. RENDER HEADER ---
st.markdown("<h1 class='main-title'>YIELD EQUILIBRIUM</h1>", unsafe_allow_html=True)
st.markdown(f"<p class='sub-header'>{hotel_id.upper()} • STRATEGIC ANALYTICS</p>", unsafe_allow_html=True)
...
with cp3: st.markdown("<div class='pillar-box'><h4>3. Efficiency Indexing</h4><p style='font-size:0.8rem;'>Measuring variance against breakeven.</p></div>", unsafe_allow_html=True)

# --- 6. SEGMENTS ---
def draw_s(title, key, d_adr, d_fl, color, is_ota=False, is_group=False):
    ...
    with c2:
        ...
        if is_group:
            ...
            mice, trans = gc1.number_input("MICE (Pax)", 0.0, key=key+"ev"), gc2.number_input("Transport", 0.0, key=key+"tr")
        st.markdown("</div>", unsafe_allow_html=True)
    res = calc_w([s,d,t], adr_v, n, mx, (ota_p if is_ota else 0.0), fl_v, mice, trans)
    if res:
        with c3:
...
draw_s("4. Corporate Groups (MICE)", "cgrp", 50, 30, "#9b59b6", False, True)
...
st.markdown(f"<div style='background:#1e3799;padding:20px;border-radius:12px;text-align:center;color:white;'><h3>Portfolio Total: {cu} {tot_w:,.2f}</h3></div>", unsafe_allow_html=True)
