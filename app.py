...
# --- MAIN ENGINE (RUNS ONLY IF AUTH IS TRUE) ---
with st.sidebar:
...
def calc_w(rms, adr, n, meals, comm, fl, mice=0.0, trans=0.0):
    ...
    # Breakdown math to avoid syntax issues
    net_after_meals = u_n - m_c_total
    commission_deduction = net_after_meals * comm
    mice_add = (mice * px_r) / (n * tx)
    
    unit_w = (net_after_meals - commission_deduction) + mice_add
    total_w = (unit_w * tot_r * n) + (trans / tx)
    d_u = total_w / (tot_r * n)
    
    hrd = fl * 1.25 if (tot_r / h_tot) >= 0.2 else fl
    l, b = ("OPTIMIZED","#27ae60") if d_u>=hrd else ("MARGINAL","#ff9800") if d_u>=hrd*0.95 else ("DILUTIVE","#e74c3c")
