...
def draw_seg(label, key, suggest_adr, floor_def, color, is_ota=False, group=False):
    ...
    with c_in:
        ...
        demand_sel = m_row[0].selectbox("Demand", ["Standard", "Compression (Peak)", "Distressed"], key=f"dm_{key}_{rk}")
...
<div class='theory-card' style='background:#f1f4f9; border: 1px solid #1e3799; padding:25px;'>
    ...
    <p style='font-size:0.92rem; color:#333; line-height:1.6;'>
        The <b>Yield Equilibrium</b> model identifies the exact point where a hotel captures maximum wealth without diluting asset value.
    </p>
    <div style='margin-top:15px;'>
        <p style='font-size:0.88rem; color:#333; margin-bottom:10px;'>
            <b>🏛️ PILLAR 01: INTERNAL WEALTH STRIPPING</b><br>
            The engine strips <b>statutory taxes ({tx_div})</b> and costs to isolate 'Net Wealth'.
        </p>
        <p style='font-size:0.88rem; color:#333; margin-bottom:10px;'>
            <b>⚖️ PILLAR 02: HURDLE EQUILIBRIUM</b><br>
            Hurdles protect peak inventory from lower-value business displacement.
        </p>
        <p style='font-size:0.88rem; color:#333;'>
            <b>🌐 PILLAR 03: EXTERNAL VELOCITY</b><br>
            Integrates OTB pace to apply a <b>Velocity Multiplier ({v_mult}x)</b>.
        </p>
    </div>
</div>
...
<form action="https://formspree.io/f/mkoywogq" method="POST" style="display: flex; flex-direction: column; gap: 15px; background: white; padding: 20px; border-radius: 10px;">
    ...
    <button type="submit" style="background-color: #1e3799; color: white; padding: 12px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold;">🚀 Submit to Developer</button>
</form>
"""
