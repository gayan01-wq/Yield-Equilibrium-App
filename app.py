...
# --- 3. SIDEBAR (MASTER PACKAGE COSTS) ---
...
with st.sidebar:
    ...
    # MASTER PACKAGE COSTS - These are the costs applied when a meal plan is selected
    st.markdown("### 🍽️ Meal Plan Cost (PP)")
    meal_package_costs = {
        "BB": st.number_input("BB Cost (PP)", min_value=0.0, value=2.00, step=0.5, key="bb_mc_"+rk),
        "HB": st.number_input("HB Cost (PP)", min_value=0.0, value=5.00, step=0.5, key="hb_mc_"+rk),
        "FB": st.number_input("FB Cost (PP)", min_value=0.0, value=10.00, step=0.5, key="fb_mc_"+rk),
        "SAI": st.number_input("SAI Cost (PP)", min_value=0.0, value=15.00, step=0.5, key="sai_mc_"+rk),
        "AI": st.number_input("AI Cost (PP)", min_value=0.0, value=20.00, step=0.5, key="ai_mc_"+rk)
    }

# --- 4. CALCULATION ENGINE ---
def run_segment_yield(adr, meal_selections, base_hurdle, demand_type, is_group, total_rooms, comm_rate=0.0, mice=0.0, laundry=0.0, transport=0.0):
    ...
    # MEAL CALCULATION: Sum (Rooms Selected for Plan * Master Cost from Sidebar)
    total_meal_outflow = sum(count * meal_package_costs[plan] for plan, count in meal_selections.items())
    
    # Normalize per room
    divisor = max(total_rooms, 1)
    meal_cost_per_room = total_meal_outflow / divisor
    ...
    # Determine display basis (Highest logic)
    mp_basis = "RO"
    for plan in ["AI", "SAI", "FB", "HB", "BB"]:
        if meal_selections.get(plan, 0) > 0:
            mp_basis = plan
            break
...
segments = [
    ...
    {"label": "3. GROUP / MICE", "key": "mice", "color": "#34495e", "hurdle": 32.0, "group": True}
]
...
for seg in segments:
    if st.checkbox(f"Activate {seg['label']}", value=True, key=f"act_{seg['key']}_{rk}"):
        ...
        with st.container():
            ...
            # Row 2: Meal Selections (Select room count per plan)
