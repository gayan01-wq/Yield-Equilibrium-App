...
with st.sidebar:
    ...
    meal_costs = {
        "RO": 0.0,
        "BB": st.number_input("BB Cost (Breakfast)", 2.5, key="cbb"+rk),
...
def draw_seg(label, key, suggest_adr, floor_def, color, is_ota=False, group=False):
    ...
    with c_in:
        ...
        m_row = st.columns(7)
        p_ro = m_row[0].number_input("RO", 0, key="ro"+key+rk); p_bb = m_row[1].number_input("BB", 0, key="bb"+key+rk); p_ln = m_row[2].number_input("LN", 0, key="ln"+key+rk); p_dn = m_row[3].number_input("DN", 0, key="dn"+key+rk); p_sai = m_row[4].number_input("SAI", 0, key="sai"+key+rk); p_ai = m_row[5].number_input("AI", 0, key="ai"+key+rk); p_snk = m_row[6].number_input("Snk", 0, key="snk"+key+rk)
