...
with st.sidebar:
    ...
    sim_rooms = st.slider("Group Inventory Shift", 5, 3000, 85, key="sim_s_"+rk)
    ...
    cur_sym = currencies[st.selectbox("Select Base Currency", list(currencies.keys()), key="c_sel_"+rk)]
...
def run_segment_yield(adr, meal_qty, hurdle, demand_type, is_group, comm_rate=0.0, mice=0.0, laundry=0.0, transport=0.0):
    ...
    group_revenue = (mice / tx_div) + ((transport / tx_div) / sim_rooms if sim_rooms > 0 else 0) if is_group else 0
...
# RESTORED MARKET INSIGHTS TOPIC LINE
st.markdown(f"""
...
# --- 8. NOI SUMMARY & RESTORED PILLAR DESCRIPTIONS ---
st.divider()
...
# THE UPGRADED THEORY BOX (PERMANENT)
...
with c_a:
    ...
    st.markdown(f"<p style='font-size:0.88rem; color:#4b6584;'>Deconstructs the Gross Rate by stripping statutory taxes ({tx_div}), commissions, and marginal meal costs to find the <b>Net-Core</b>.</p>", unsafe_allow_html=True)
with c_b:
    st.markdown("<span class='pillar-header'>⚖️ Pillar 02: Hurdle Equilibrium</span>", unsafe_allow_html=True)
    st.markdown("<p style='font-size:0.88rem; color:#4b6584;'>Protects inventory from displacement by establishing dynamic floors. Uses the <b>Group Simulation</b> to stress-test high-volume requests.</p>", unsafe_allow_html=True)
with c_c:
    st.markdown("<span class='pillar-header'>🌐 Pillar 03: External Velocity</span>", unsafe_allow_html=True)
    st.markdown("<p style='font-size:0.88rem; color:#4b6584;'>Integrates real-time Market Pulse and Aviation data to apply a <b>Velocity Multiplier</b> (0.85x to 1.25x) based on demand intensity.</p>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)
