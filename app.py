...
with st.sidebar:
    ...
    meal_bb = st.number_input("BB Cost", value=5.0, key="bb_mc_"+rk)
    meal_hb = st.number_input("HB Cost", value=10.0, key="hb_mc_"+rk)
    meal_fb = st.number_input("FB Cost", value=15.0, key="fb_mc_"+rk)
    meal_sai = st.number_input("SAI Cost", value=20.0, key="sai_mc_"+rk)
    meal_ai = st.number_input("AI Cost", value=25.0, key="ai_mc_"+rk)
...
def run_yield(rms_list, adr, hurdle, demand_type, meals, is_ota=False):
    ...
    # Calculate weighted meal costs
    total_meal_cost = (meals['BB']*meal_bb) + (meals['HB']*meal_hb) + (meals['FB']*meal_fb) + (meals['SAI']*meal_sai) + (meals['AI']*meal_ai)
...
def ask_ai_equilibrium(user_query, context_data):
    try:
        ...
        # Fix: Using 'gemini-1.5-flash' for standard stability
        model = genai.GenerativeModel('gemini-1.5-flash') 
        response = model.generate_content(f"Audit this revenue data based on Yield Equilibrium pillars: {context_data}. Prompt: {user_query}")
