...
with st.sidebar:
 ...
 hotel = st.text_input("Property", "Wyndham Garden Salalah"); h_tot = st.number_input("Total Inventory", 1, 5000, 237)
 cur_list = ["OMR","AED","SAR","KWD","BHD","QAR","EGP","JOD","LKR","INR","THB","SGD","MYR","CNY","JPY","HKD","IDR","EUR","GBP","CHF","USD"]
 cu = st.selectbox("Currency Selection", cur_list)
...
def draw_s(title, key, d_adr, d_fl, color, is_o=False, is_g=False):
 ...
 if res:
  with c3:
   st.metric("Net Wealth", f"{cu} {res['u']:,.2f}"); st.markdown(f"<div class='status-box' style='background:{res['b']}'>{res['l']}</div>", unsafe_allow_html=True); st.caption(res['msg']); st.markdown(f"<div class='exposure-bar'>{res['rn']} RNs | Total: {res['tot']:,.0f}</div>", unsafe_allow_html=True); st.session_state[key+"_t"] = res['tot']
 st.divider()
