...
# --- 7. DETAILED METHODOLOGY & YIELD EQUILIBRIUM PILLARS ---
st.divider()
...
<div class='theory-card' style='background:#f1f4f9; border: 1px solid #1e3799; padding:25px;'>
    ...
    <div style='margin-top:15px;'>
        ...
        Gross revenue is an illusion. The tool deconstructs every deal by stripping away <b>statutory taxes ({tx_div})</b>, <b>distribution leakages (OTA Commissions)</b>, and <b>marginal production costs (F&B unit costs)</b>. The result is 'Net Wealth'—the only metric that impacts the bottom line.</p>
        
        <p style='font-size:0.88rem; color:#333;'><b>⚖️ PILLAR 02: HURDLE EQUILIBRIUM (THE DISPLACEMENT GUARD)</b><br>
        Strategy is determined by context. By selecting <b>Demand Contexts</b>, the tool shifts the hurdle equilibrium. During 'Compression', the tool inflates hurdles to protect inventory for high-value segments, preventing lower-value business from displacing future yield.</p>
        
        <p style='font-size:0.88rem; color:#333;'><b>🌐 PILLAR 03: EXTERNAL VELOCITY (THE MARKET PULSE)</b><br>
        Static pricing is an asset risk. This pillar integrates <b>Market Intelligence</b>—Aviation rotations, local events, and OTB pace—to apply a <b>Velocity Multiplier ({v_mult}x)</b>, ensuring pricing remains in constant equilibrium with the market.</p>
    </div>
</div>
...
with c_a:
    st.markdown(f"<div class='theory-card'><b>🏗️ Pillar 01: Execution</b><br>Wealth stripping via Tax, segment-specific Commissions (**OTA only**), and specialized costs.</div>", unsafe_allow_html=True)
with c_b:
    st.markdown(f"<div class='theory-card'><b>🌐 Pillar 03: Execution</b><br>ADW Pace vs Benchmark multiplier (**{v_mult}x**) substantiated by news & aviation rotations.</div>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)
