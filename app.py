...
.metric-box{background:#1e3799; color:white; padding:15px; border-radius:10px; text-align:center; height: 100%; display: flex; flex-direction: column; justify-content: center;}
...
def draw_seg(label, key, suggest_adr, floor, is_ota=False):
    ...
    # Grid layout: 3 parts inputs, 1 part metrics
    col_inputs, col_metrics = st.columns([3, 1])
    ...
    with col_metrics:
        if res:
            ...
            <div class='metric-box'>
                <small style='opacity:0.8;'>Net Wealth Yield</small>
                <div style='font-size:1.6rem; font-weight:bold;'>{cur_sym}{res['w']:,.2f}</div>
            </div>
            <div class='status-tag' style='background:{res['cl']}'>{res['st']}</div>
            """, unsafe_allow_html=True)
        else:
            st.info("Waiting for room input...")
...
<div style='background:#f1f4f9; padding:20px; border-radius:15px; border:1px solid #1e3799;'>
    <h4 style='color:#1e3799; text-align:center; margin-top:0;'>RESEARCH FRAMEWORK: YIELD EQUILIBRIUM</h4>
...
st.markdown("<br><div style='text-align:center; color:#4b6584; font-size:0.8rem;'>System Developer: Gayan Nugawela | gayan01@gmail.com</div>", unsafe_allow_html=True)
