...
st.caption("Developed by Gayan Nugawela | Surgical Audit Engine")

# --- SIDEBAR SETTINGS ---
with st.sidebar:
    st.header("1. Meal Net Costs")
    c_bb = st.number_input("Breakfast", 5.0)
    c_lh = st.number_input("Lunch", 7.0)
    c_dr = st.number_input("Dinner", 10.0)
    c_sai = st.number_input("SAI Supplement", 8.0)
    c_ai = st.number_input("AI Supplement", 15.0)
...
def audit(s, d, t, adr, counts, comm, floor):
    paid = s + d + t
    if paid <= 0: return {"rp": 0, "un": 0, "st": "WAITING FOR ROOMS", "cl": "gray", "tfb": 0}
