...
with st.sidebar:
    ...
    # Mapping logic correctly sealed
...
st.markdown("<div class='definition-box'><b>The Yield Equilibrium Framework:</b> Calculating <b>Real Bankable Wealth</b> by stripping taxes, commissions, and variable meal allocations to protect bottom-line efficiency.</div>", unsafe_allow_html=True)
st.divider()

# --- 6. RENDER SEGMENTS (Hardened for Visibility) ---
all_res = []
...
def draw_seg(title, key, d_adr, d_fl, color, is_ota=False, is_grp=False):
    ...
    else: st.info("Input inventory to see wealth data...")
    st.divider()

# Rendering order is strictly forced here
