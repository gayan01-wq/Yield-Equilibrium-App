...
if check_password():
    ...
    st.markdown("""
        ...
        .stMetric { background: white; border-radius: 10px; padding: 10px; border: 1px solid #cce5ff; }
        ...
        .status-pill { padding: 4px 10px; border-radius: 15px; color: white !important; 
    ...
    with st.sidebar:
        ...
        st.markdown("**MBA | CRME | CHRM | RevOps**")
        st.divider()
    ...
    def run_calc(rms, adr, nts, mix, cp, hurdle, ev_r=0, tr_c=0):
        ...
        # Shortened calculation lines to prevent truncation
    ...
    def seg_ui(name, clr, ky, r_def, h_def, comm, is_g=False):
        st.markdown(f"<div class='card' style='border-left-color:{clr}'>{name}</div>", unsafe_allow
