...
with st.sidebar:
    ...
    m_ln = st.number_input("Lunch (LN)", value=4.0) # NEW: Lunch added
    m_dn = st.number_input("Dinner (DN)", value=6.0)
    m_sai = st.number_input("SAI Full", value=20.0)
    m_ai = st.number_input("AI Full", value=27.0)
    
    # Updated mapping to include Lunch
    m_map = {
        "RO": 0.0, 
        "BB": m_bb, 
        "HB": m_bb + m_dn, 
        "FB": m_bb + m_ln + m_dn, # Updated FB to BB + LN + DN
        "SAI": m_sai, 
        "AI": m_ai
    }
...
st.markdown("<div class='definition-box'><b>The Yield Equilibrium Framework:</b> Calculating <b>Real Bankable Wealth</b> by stripping taxes, commissions, and all meal basis allocations.</div>", unsafe_allow_html=True)
st.divider()
