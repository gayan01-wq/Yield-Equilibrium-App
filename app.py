...
# --- 1. SETTINGS & STYLING ---
st.set_page_config(layout="wide", page_title="Yield Equilibrium Center")
...
if check_password():
    ...
    with st.sidebar:
        ...
        st.caption("Standard costs applied based on Salalah Market norms.")
        
        st.divider()
        st.subheader("📈 Density Logic")
        density_trigger = 20.0  # The 20% Goldilocks Zone
        risk_premium = 0.15     # 15% Hurdle increase for high density
        currency = st.selectbox("Currency", ["OMR", "AED", "USD", "LKR"])
    ...
    def calculate_wealth(rms, adr, nts, mq, comm, hurdle, ev_p=0):
        ...
        # Velocity & Density Adjustment
        applied_hurdle = hurdle
        density_msg = "✅ Stable Density"
        
        if density >= 20.0:
            applied_hurdle = hurdle * (1 + risk_premium)
            density_msg = "🚨 HIGH DENSITY RISK (>20%)"
    ...
    def render_segment(name, color, bg, pref, def_adr, def_hurdle, comm, is_group=False):
        ...
        with c3:
            if res:
                ...
                st.write(f"Inventory Impact: **{res['density']:.1f}%**")
                st.caption(res['msg'])
                st.write(f"Hurdle Target: **{res['hurdle']:.2f}**")
    ...
    st.divider()
    s2 = render_segment("MICE & Groups", "#2c3e50", "#eceff1", "gr", 50, 35, 0.0, is_group=True)
