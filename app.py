...
st.markdown("""
    ...
    .pillar-box { background-color: #f1f4f9; padding: 20px; border-radius: 10px; border-top: 4px solid #3498db; min-height: 200px; }
...
with st.sidebar:
    ...
    # Expanded Currency List: ME, Asia, Europe
    currency_list = sorted([
        "OMR", "AED", "SAR", "QAR", "BHD", "KWD", # Middle East
        "USD", "EUR", "GBP", "CHF", "SEK", "NOK", # Europe/Global
        "INR", "LKR", "THB", "SGD", "MYR", "CNY", "JPY", "KRW", "IDR" # Asia
    ])
    cu = st.selectbox("Currency", currency_list)
...
def get_wealth(rooms, adr, nights, meal_plan, commission, floor_price):
    ...
    if unit_wealth < (hurdle * 0.8) or unit_wealth <= 0:
        ...
        desc = "REJECT: High inventory risk with zero bottom-line contribution."
    elif unit_wealth < hurdle:
        label, color, bg = "MARGINAL", "#2c3e50", "#f1c40f"
        desc = "FILL ONLY: Low efficiency. Accept only to sustain cash flow in low demand."
    else:
        label, color, bg = "OPTIMIZED", "#FFFFFF", "#27ae60"
        desc = "ACCEPT: High-efficiency wealth generator. Priority booking."
        
    return {"u": unit_wealth, "label": label, "color": color, "bg": bg, "total": total_wealth, "gross": gross_total, "util": utilization, "eff": eff, "desc": desc}
